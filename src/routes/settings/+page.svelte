...
<div class="flex flex-col gap-2">
    <div class="form-control max-w-64">
        ...
        <div class="label">
            <span class="label-text">{ $t("page-settings-language")}</span>
        </div>
            <select class="select select-bordered" value={$language} onchange={(ev) => language.set(ev.currentTarget.value as Language)}>
                {#each languageValues as lang}
                    <option value={lang}>{ $t(`page-settings-language-${lang}`)} ({nativeLanguageName(lang)})</option>
                {/each}
            </select>
        </label>
    </div>
    ...
    <div class="form-control max-w-64">
        <label class="label cursor-pointer">
            <span class="label-text">{ $t("page-settings-use-dark-mode") }</span>
    ...
    <div class="form-control max-w-64">
        <label class="label cursor-pointer">
            <span class="label-text">{ $t("page-settings-view-weapon-abilities") }</span>
    ...
    <div class="form-control max-w-64">
        <label class="label cursor-pointer">
            <span class="label-text">{ $t("page-settings-view-weapon-talents") }</span>
