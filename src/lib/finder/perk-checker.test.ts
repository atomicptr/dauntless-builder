...
test("basicPerkChecker1", () => {
    ...
    expect(
        findAvailablePerksImplementation(
            selectedPerks,
            requestedPerks,
            phalanxFinderData,
            phalanxData.perks,
...
test("requestAllPerks", () => {
    const selectedPerks: number[] = [];
    const requestedPerks = Object.values(phalanxData.perks).map((perk) => perk.id);
    expect(
        findAvailablePerksImplementation(
            selectedPerks,
            requestedPerks,
...
test("largeQuantityPerkChecker", () => {
    ...
    let requestedPerks = Object.values(phalanxData.perks).map((perk) => perk.id);
